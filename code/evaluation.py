...
def lreg_err(group, error):
    '''
    helper function for create_cross_county_comparison
    '''
    ...
    if group.shape[0] < 100:
        ...
        try: 
            X = np.log(group['assessed']).values.reshape(-1, 1)
            y = np.log(group['sale'])
            ...
            if error == 'mpe':
                ...
                result = mape(preds, y)
            elif error == 'r_squared':
                result = r_squared(preds, y)
            else:
                result = np.nan
